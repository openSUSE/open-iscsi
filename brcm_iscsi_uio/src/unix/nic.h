...
struct pci_device_id {
	...
	const uint32_t subvendor, subdevice;  /* Subsystem ID's or PCI_ANY_ID */
...
struct nic_lib_ops {
	...
	void (*get_uio_name)(char **uio_name,
...
#define NIC_RUNNING	0x0002
#define NIC_LONG_SLEEP	0x0004
#define NIC_EXIT	0x0008
	...
	/*  Wait for the particular uio device to appear */
	pthread_mutex_t  uio_wait_mutex;
...
void put_packet_in_tx_queue(struct packet *pkt,
                            nic_t *nic);
...
int nic_packet_capture(struct nic *,
                       struct packet *pkt);
