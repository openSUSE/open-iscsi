 ...
 */
int parse_vlan_table(struct vlan_handle *handle,
		     char *raw, uint32_t raw_size)
{
	FILE *fp;
	int i;
	char *token;
	...
	if(handle->entries == NULL) {
		LOG_ERR("Couldn't malloc space to parse vlan table. entires: %d size: %d",
	...
	while(1)
	{
		...
		r = fscanf(fp, "%15s |%hu |%15s",
				...
				&entry->vlan_id,
				entry->phy_iface_name);
		if (r != 3) {
			if (feof(fp)) { /* EOF with no (nonspace) chars read. */
				break;
			}

			LOG_WARN("Parsing error: parsed %d elements",
				 r);
			break;
		}
