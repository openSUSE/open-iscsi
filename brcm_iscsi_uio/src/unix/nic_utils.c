...
static const char iscsi_host_path_netdev_template[] = "/sys/class/iscsi_host/host%d/netdev";
 ...
 *****************************************************************************/
static int nic_util_enable_disable_multicast(nic_t *nic,
					     uint32_t cmd)
{
	int rc = 0;
					...
					size_t name_size)
{
	...
	for(path_iterator = 0;
	    ...
	    path_iterator++) {
		/*  Build the path to determine uio name */
		rc = sprintf(path, search_paths[path_iterator],
			     uio_minor);
		count = scandir(path, &files,
				search_filters[path_iterator],
				alphasort);
		...
		default:
			LOG_WARN(PFX "Too many entries when looking for device: %s",
				 path);
...
int from_phys_name_find_assoicated_uio_device(nic_t *nic)
{
	...
	case -1:
		LOG_WARN(PFX "Error when scanning for %s in path: %s [%s]",
			 interface_name,
			 base_uio_sysfs_name, strerror(errno));
		return -EINVAL;
	}
	...
	LOG_WARN("Could not find assoicate uio device with %s",
		 interface_name);
...
int nic_fill_name(nic_t *nic)
{
	...
	if((nic->config_device_name != NULL) &&
	   (memcmp(uio_base_dir, nic->config_device_name,
		...
		if(rc < 0) {
			/*  TODO wait for ethernet interface to appear */
			LOG_ERR(PFX "Could not determine UIO name");
		}
...
void prepare_nic(nic_t *nic)
{
	...
	LOG_INFO("%s: found NIC '%s'",
		 nic->log_name,
		 nic->pci_id->device_name);
...
int nic_enable(nic_t *nic)
{
	...
	{
		LOG_INFO(PFX "%s: device already enabled: flag: "
			     "0x%x state: 0x%x",
 ...
 *                      function will traverse through the local VLAN table
 *                      determine if the associated interface is a VLAN
 *                      then will create network interfaces for all VLAN ID
 *  @param dev - struct cnic_uio device to assign the VLAN ID
 *  @return 0 on success, <0 on failure
 */
int add_vlan_interfaces(nic_t *nic)
{
	...
	if(rc != 0) {
		...
		rc = -EIO;
		goto error_vlan_handle;
	}

        rc = init_vlan_found_handle(&found_handle, &handle);
	if(rc != 0) {
		LOG_ERR(PFX "Failed to init VLAN found handle");
		goto error_vlan_found_handle;
	}

	rc = find_vlans_using_phy_interface(&handle,
					    &found_handle,
					    nic->eth_device_name);
	if(rc > 0)
	{
		...
		for(i=0; i<rc; i++)
		{
			struct vlan_found_entry *found_entry;
			nic_interface_t *nic_iface;

			found_entry = &found_handle.entries[i];
			...
			if(found_entry->found == VLAN_ENTRY_FOUND)
			{
				...
				nic_iface->vlan_id = found_handle.handle->entries->vlan_id;

				rc = nic_add_nic_iface(nic, nic_iface);
			}
		}
	}

	release_vlan_found_handle(&found_handle);

error_vlan_found_handle:
 ...
 *  @return the packet in hte queue
 */
void put_packet_in_tx_queue(packet_t *pkt,
			    nic_t *nic)
{
	return put_packet_in_queue(pkt,
				   &nic->tx_packet_queue);
}
 ...
 *  @return the packet in hte queue
 */
void put_packet_in_free_queue(packet_t *pkt,
			    nic_t *nic)
{
	pthread_mutex_lock(&nic->free_packet_queue_mutex);
	put_packet_in_queue(pkt,
			     &nic->free_packet_queue);
	pthread_mutex_unlock(&nic->free_packet_queue_mutex);
}
...
int determine_file_size_read(const char *filepath) 
{
	...
	while(size > 0)
	{
		...
		case 0:
			break;
		case -1:
			LOG_ERR("Error reading file to determine size: %s",
				strerror(errno));
			total_size = -1;
			break;
		default:
...
int capture_file(char **raw, uint32_t *raw_size, const char *path)
{
	...
	if(file_size < 0)
	{
		LOG_ERR("Could not determine size %s",
			path);
		return -EIO;
	}
	...
	*raw = malloc(file_size);
	...
	if(read_size < 0) {
		LOG_ERR("Could not read capture, path: %s len: %d",
			path, file_size);
		free(*raw);
		*raw = NULL;
		rc = errno;
